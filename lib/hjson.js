    ...
    return function (source, options) {
      ...
      while (ch) {
        ...
        if (ch) {
          /* Syntax error.
           *
           * If `source` begins with on of the special characters indicating
           * object, array or string, call it an error.
           */
          if (source.match(/^\s*[\[{"]/)) { error("Syntax error"); }    // }

          /* Otherwise, adjust `source` to be a simple object, reset our parse
           * state and try one more time.
           *
           * This allows an entire file to be treated as a simple object
